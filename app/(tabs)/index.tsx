...
type ScoreEntry = {
  ...
  created_at: string;
  player_id?: string;
  played?: boolean; // Status za rumeno piko
};
...
export default function ActiveGame() {
  ...
  const [scorePlayed, setScorePlayed] = useState(false); // Za rumeno piko
  ...
  const openScoreInput = (playerId: string) => {
    ...
    setScoreInput('');
    setScorePlayed(false); // Reset
    setShowScoreModal(true);
  };

  // AutoFocus samo za točke (ne spreminjamo, kot si želel)
  useEffect(() => {
    if (showScoreModal) {
  ...
  const submitScore = async () => {
    ...
    try {
      const { error } = await supabase.from('score_entries').insert({
        ...
        game_id: gameId, 
        points,
        played: scorePlayed // Shranimo "je igral"
      });
        ...
        <View style={styles.modalOverlay}>
          <View style={[styles.modalContent, { height: '90%', maxHeight: '90%' }]}>
            ...
            <View style={styles.searchContainer}>
                ...
                {/* POPRAVEK: Tipkovnica se NE odpre sama */}
                <TextInput
                    ...
                    // autoFocus vrstica je IZBRISANA
...
const styles = StyleSheet.create({
  ...
  // Radelc slogi (popravljeni v prejšnjem koraku)
