                                ...
                                <TouchableOpacity key={stat.name} style={styles.leaderboardItem} onPress={() => openGlobalPlayerDetails(stat)}>
                                    <View style={styles.rankBox}>
                                        {rank === 1 ? <Trophy size={28} color="#ffd700" /> :
                                         rank === 2 ? <Trophy size={28} color="#c0c0c0" /> :
                                         rank === 3 ? <Trophy size={28} color="#cd7f32" /> :
              ...
              {gamePlayers.map((player, index, array) => {
                ...
                return (
                  <LinearGradient colors={['#252525', '#1e1e1e']} key={player.id} style={styles.playerRowContainer}>
                    ...
                    {/* POPRAVEK: Radelci logike - črn s belim robom za uporabljenega */}
...
const styles = StyleSheet.create({
  ...
  // LEADERBOARD STYLES - POPRAVLJENO ZA LEVO PORAVNAVO
  ...
  // POPRAVEK: justifyContent: 'center' za navpično poravnavo imena
  leaderboardInfo: { flex: 1, paddingLeft: 12, justifyContent: 'center' },
  leaderboardName: { color: '#fff', fontSize: 17, fontWeight: '700' },
  leaderboardSub: { color: '#666', fontSize: 13 },
  medalRow: { flexDirection: 'row', gap: 6, alignItems: 'center' },
  
  // POPRAVEK: Odstranjen paddingVertical, dodan paddingHorizontal
  medalTag: { flexDirection: 'row', paddingHorizontal: 8, alignItems: 'center', justifyContent: 'center', borderRadius: 6, borderWidth: 1, backgroundColor: '#111' },
  medalText: { fontSize: 15, fontWeight: '800', marginLeft: 4 },
  ...
  radelcUnused: { borderWidth: 2, borderColor: '#888', backgroundColor: 'transparent' },
  radelcUsed: { backgroundColor: '#000000', borderWidth: 1, borderColor: '#fff' },
